...
from acm_opt.tiers import load_tier_spec

_NMOS = {"type": 1}
_EXPLORERS = frozenset({"acm_x", "qlaw_x"})
...
def default_models(repo_root: Path) -> dict[str, ModelSpec]:
    ...
    if "acm4cj" in out and "acm_x" not in out:
        out["acm_x"] = _tier_model(
            repo_root,
            name="acm_x",
            tier_id="acm4cj",
            spice_model="ACM_X",
            doc_path=repo_root / "models/acm_x/README.md",
            experimental=True,
            stage="C",
        )
    if "qlaw" in out and "qlaw_x" not in out:
        out["qlaw_x"] = _tier_model(
            repo_root,
            name="qlaw_x",
            tier_id="qlaw",
            spice_model="QLAW_X",
...
def qlaw_models(repo_root: Path) -> list[ModelSpec]:
    """Return all registered generation-6 QLAW physics cores (excludes explorers)."""
    return [
        m
        for m in default_models(repo_root).values()
        if m.physics_tier is not None
        and m.name not in _EXPLORERS
        and m.name.startswith("qlaw")
    ]
